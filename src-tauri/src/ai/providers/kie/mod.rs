...
const UPLOAD_PATH: &str = "images/storyboard-science";
...
impl KieProvider {
    pub fn new() -> Self {
        Self {
            client: Client::builder()
                .timeout(Duration::from_secs(3600)) // 设置3600秒超时，与服务器端保持一致
    ...
    fn max_reference_count_for_model(model: &str) -> usize {
        if model == "nano-banana-pro" {
            8
        } else if model == "gemini-omni-video" {
            7
        } else {
            14
        }
    ...
    pub async fn upload_ref_image(
        ...
        let bytes = Self::source_to_bytes(source).map_err(|err| {
            AIError::InvalidRequest(format!(
                "Failed to read reference image for 小鸭 upload: {}; source={}",
                err, source
            ))
        })?;
        ...
        let body = serde_json::from_str::<Value>(&raw_response).map_err(|err| {
            AIError::Provider(format!(
                "小鸭 file upload invalid JSON response: {}; raw={}",
                err,
                raw_response
            ))
        })?;
        ...
        let uploaded_url = Self::extract_uploaded_file_url(&body).ok_or_else(|| {
            AIError::Provider(format!("小鸭 file upload missing fileUrl, raw response: {}", body))
        })?;

        if !Self::is_http_url(&uploaded_url) {
            return Err(AIError::Provider(format!(
                "小鸭 upload returned non-http URL: {}, raw response: {}",
                uploaded_url, body
            )));
        }
    ...
    async fn upload_reference_images(
        ...
        for (index, source) in capped.into_iter().enumerate() {
            uploaded_urls.push(self.upload_ref_image(api_key, source, index).await?);
        }
        Ok(uploaded_urls)
    }
    ...
    async fn create_task(
        ...
        if uploaded_images.iter().any(|url| !Self::is_http_url(url)) {
            return Err(AIError::InvalidRequest(
                "小鸭 image reference contains non-http URL, upload step may have failed".to_string(),
            ));
        }

        let is_seedream = model.starts_with("seedream/");
        let is_gpt_image = model.starts_with("gpt-image");
        let is_gemini_omni_video = model == "gemini-omni-video";
        let is_grok_video = model == "grok-imagine/image-to-video";

        info!(
            "[小鸭 createTask] using uploaded image URLs: count={}",
            uploaded_images.len()
        );
        ...
        let input = if is_gemini_omni_video {
            let duration = request
                ...
                .unwrap_or(4) as u32;
            let video_urls: Vec<String> = request
                .extra_params
                .as_ref()
                .and_then(|params| params.get("video_input"))
            ...
            if !uploaded_images.is_empty() {
                input["image_urls"] = json!(uploaded_images);
            }
            if !video_urls.is_empty() {
                input["video_input"] = json!(video_urls);
            }
            input
        } else if is_grok_video {
            let duration = request
                .extra_params
                .as_ref()
                .and_then(|params| params.get("duration_seconds"))
                .and_then(|raw| raw.as_u64())
                .unwrap_or(6) as u32;
            let grok_task_id = format!("grok_{}", Uuid::new_v4());
            ...
            let quality = match request.size.as_str() {
                "1K" => "basic",
                _ => "high",
            };
            json!({
        ...
        let body = serde_json::from_str::<KieCreateTaskResponse>(&raw_response).map_err(|err| {
            AIError::Provider(format!(
                "小鸭 createTask invalid JSON response: {}; raw={}",
                err,
                raw_response
            ))
        })?;
        ...
        body.data
            ...
            .ok_or_else(|| AIError::Provider("小鸭 createTask missing taskId".to_string()))
    }
    ...
    async fn poll_task_once(
        ...
        let body = serde_json::from_str::<KieTaskInfoResponse>(&raw_response).map_err(|err| {
            AIError::Provider(format!(
                "小鸭 recordInfo invalid JSON response: {}; raw={}",
                err,
                raw_response
            ))
        })?;
        if body.code != 200 {
            ...
            return Err(AIError::Provider(format!("小鸭 task query rejected: {}", message)));
        }

        let data = body
            .data
            .ok_or_else(|| AIError::Provider("小鸭 task query missing data".to_string()))?;
        match data.state.as_deref() {
            Some("success") => {
                let result_json = data.result_json.ok_or_else(|| {
                    AIError::Provider("小鸭 success response missing resultJson".to_string())
                })?;
                if let Some(url) = Self::extract_result_url(&result_json) {
                    return Ok(ProviderTaskPollResult::Succeeded(url));
                }
                Err(AIError::Provider(
                    "小鸭 resultJson has no valid result URL".to_string(),
                ))
            }
            Some("fail") => Ok(ProviderTaskPollResult::Failed(
                data.fail_msg.unwrap_or_else(|| "小鸭 task failed".to_string()),
            )),
            Some("waiting") | Some("queuing") | Some("generating") | None => {
                Ok(ProviderTaskPollResult::Running)
            }
            Some(other) => Err(AIError::Provider(format!("小鸭 unexpected task state: {}", other))),
        }
    }
...
impl AIProvider for KieProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        matches!(
            ...
            "nano-banana-2" | "nano-banana-pro" | "seedream/5-lite-image-to-image" | "gpt-image-2-image-to-image" | "gemini-omni-video" | "grok-imagine/image-to-video"
        )
    }
    ...
    fn list_models(&self) -> Vec<String> {
        vec![
            ...
            "kie/seedream/5-lite-image-to-image".to_string(),
            "kie/gpt-image-2-image-to-image".to_string(),
            "kie/gemini-omni-video".to_string(),
            "kie/grok-imagine/image-to-video".to_string(),
        ]
    }
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<String, AIError> {
        ...
        let model = Self::sanitize_model(&request.model);
        info!(
            "[小鸭 Request] model: {}, size: {}, aspect_ratio: {}, refs: {}",
            model,
            request.size,
