...
impl AIProvider for BaiduProvider {
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<String, AIError> {
        ...
        let ref_prefix = if ref_count > 0 {
            "\n[参考图] 附图中的参考图是你唯一的视觉来源。你必须严格复制参考图中所有视觉元素（主体外貌/毛发/场景/风格/色彩），不可修改或添加。"
        } else {
            ""
        };
...
/// 特别识别安全审核拦截（safety/moderation），提示用户修改提示词。
fn translate_baidu_image_error(raw_response: &str) -> String {
    let lower = raw_response.to_lowercase();

    // 内容安全审核拦截：提示词/参考图触发平台审核（safety_violations / moderation_blocked）
