...
const SKILL_VERSION_URL: &str = "https://aixiaoxi.top/jy/uploads/install_guide/files/version_science.txt";
...
fn get_skill_dir() -> Result<std::path::PathBuf, String> {
    let home = directories::UserDirs::new()
        ...
        .home_dir()
        .to_path_buf();
    Ok(home.join(".claude/skills/xiaoya-ai-cinema-science"))
}
...
pub(crate) async fn chat_send_message(
    ...
    system_prompt.push_str(
        ...
        [5] If user said \"A\" or \"A-生成\": output 【分镜提示词】+ grid frames per step 8. Frames start from numbered items (1、2、…6、) — NO style anchor/camera/description headers.\n\
          ...
          shots[].shot = shot number. shots[].time = the shot's start timestamp (MM:SS.mmm), exactly matching the video prompt's \"[Shot N] At MM:SS.mmm\". shots[].frames = grid frame numbers (1-6) this shot covers. shots[].camera/sound/bgm = from video prompt.\n\
...
/// Read local skill version from ~/.claude/skills/xiaoya-ai-cinema-science/version.txt
fn read_local_skill_version() -> Result<String, String> {
    let home = directories::UserDirs::new()
        .ok_or("无法解析用户主目录")?
        .home_dir()
        .to_path_buf();
    let version_path = home.join(".claude/skills/xiaoya-ai-cinema-science/version.txt");
...
pub(crate) async fn perform_skill_upgrade() -> Result<SkillUpgradeInfo, String> {
    ...
    let target_dir = skill_dir.join("xiaoya-ai-cinema-science");

    let zip_url = format!(
        "https://aixiaoxi.top/jy/uploads/install_guide/files/xiaoya-ai-cinema-science.zip"
    );
    ...
    if let Ok(content) = tokio::fs::read_to_string(&skill_md_path).await {
        let marker_prefix = "<!-- SECURITY_MARKER: xiaoya-ai-cinema-science-protected-skill-v";
        if let Some(start) = content.find(marker_prefix) {
            if let Some(end) = content[start..].find(" -->") {
                let new_marker = format!(
                    "<!-- SECURITY_MARKER: xiaoya-ai-cinema-science-protected-skill-v{} -->",
                    server_version
                );
                let updated = format!(
                    "{}{}{}",
