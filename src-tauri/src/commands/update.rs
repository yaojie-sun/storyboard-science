...
const VERSION_CHECK_URL: &str = "https://aixiaoxi.top/jy/uploads/app/version_science.json";
const GRID_PROMPT_RULES_URL: &str = "https://aixiaoxi.top/jy/uploads/app/grid_prompt_rules_science.json";
...
fn resolve_video_gen_rules_url(model: Option<String>) -> String {
    ...
    let file = match model.as_str() {
        "happyhorse_r2v" | "happyhorse/happyhorse-1.1-r2v" => "video_gen_rules_science.json",
        "science_r2v" | "science-1.0-r2v" => "video_gen_rules_science.json",
        "pixverse_c1" | "pixverse/c1" => "video_gen_rules_pixverse_c1.json",
        _ => "video_gen_rules_science.json",
    };
...
fn installer_name(version: &str) -> String {
    ...
    {
        format!("Storyboard-Science_{}_x64-setup.exe", version)
    }
    #[cfg(target_os = "macos")]
    {
        format!("Storyboard-Science_{}_universal.dmg", version)
    }
