...
fn resolve_log_dir() -> Option<PathBuf> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        candidates.push(PathBuf::from(home).join("Library/Logs/storyboard-science"));
    }

    candidates.push(std::env::temp_dir().join("storyboard-science/logs"));
...
fn setup_logging() {
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| "debug,storyboard_science=trace,tauri=debug,reqwest=debug".into());
    ...
    info!("Storyboard Science starting...");
}
