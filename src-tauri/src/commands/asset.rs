...
/// 进行中的读图结果合并缓存：asset_id → 共享的读图结果。
/// 同一 asset 的并发读图合并为一次（首个调用者读图并 set 结果，后续调用者 await 复用同一结果），
/// 既避免重复计费，又保证等待方（前端 describe_asset 补读）也能拿到描述。
/// 失败不缓存：读图失败时 get_or_try_init 返回 Err 且不 set，后续调用可重试。
