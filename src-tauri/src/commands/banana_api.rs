...
async fn send_banana_api_request<T: for<'de> Deserialize<'de>>(
    ...
    // 添加User-Agent头
    request_builder = request_builder.header("User-Agent", "Storyboard-Science/1.0");
    ...
    let response = match request_builder.send().await {
        ...
        Err(e) => {
            ...
            retry_request_builder = retry_request_builder.header("User-Agent", "Storyboard-Science/1.0");
...
pub async fn banana_login(
    ...
    let response = client
        .post(&url)
        .header("User-Agent", "Storyboard-Science/1.0")
...
/// 获取 xiaoya-ai-cinema-science auth_cache.json 路径
fn get_auth_cache_path() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home)
        .join(".claude")
        .join("skills")
        .join("xiaoya-ai-cinema-science")
        .join("auth_cache.json"))
}

/// 下载并解压 xiaoya-ai-cinema-science skill zip 到 ~/.claude/skills/（仅当目录不存在时）
...
async fn sync_xiaoya_skill(skills_dir: &std::path::Path) {
    let target_dir = skills_dir.join("xiaoya-ai-cinema-science");
    let local_version_path = target_dir.join("version.txt");
    let version_url = format!("{}/jy/uploads/install_guide/files/version_science.txt", BANANA_API_BASE_URL);
    ...
    let zip_url = format!("{}/jy/uploads/install_guide/files/xiaoya-ai-cinema-science.zip", BANANA_API_BASE_URL);

    // 下载 zip
    let response = match reqwest::get(&zip_url).await {
        Ok(r) => r,
        Err(e) => {
            warn!("下载 xiaoya-ai-cinema-science.zip 失败: {}", e);
            return;
        }
    };
    let bytes = match response.bytes().await {
        Ok(b) => b,
        Err(e) => {
            warn!("读取 xiaoya-ai-cinema-science.zip 失败: {}", e);
            return;
        }
    };
    ...
    let mut archive = match zip::ZipArchive::new(cursor) {
        Ok(a) => a,
        Err(e) => {
            warn!("打开 xiaoya-ai-cinema-science.zip 失败: {}", e);
            return;
        }
    };
    ...
    if let Err(e) = tokio::fs::create_dir_all(&target_dir).await {
        warn!("创建 xiaoya-ai-cinema-science 目录失败: {}", e);
        return;
    }
    ...
    info!("已同步 xiaoya-ai-cinema-science skill 到 {:?}", target_dir);
}
...
async fn sync_post_login_tasks(api_key: &str) -> Result<(), String> {
    ...
    // 同步 xiaoya-ai-cinema-science skill（下载并解压 zip 到 ~/.claude/skills/）
    if let Ok(claude_dir) = get_claude_dir() {
        let skills_dir = claude_dir.join("skills");
        sync_xiaoya_skill(&skills_dir).await;
    }

    // 同步 xiaoya-ai-cinema-science 的 auth_cache.json
            ...
            if let Ok(mut auth_json) = serde_json::from_str::<serde_json::Value>(&auth_content) {
                ...
                if let Some(obj) = auth_json.as_object_mut() {
                    ...
                    if let Ok(updated_auth) = serde_json::to_string_pretty(&auth_json) {
                        ...
                        info!("已同步 xiaoya-ai-cinema-science auth_cache.json");
                    }
                }
            }
        }
